...
import {
  getAsignacionesServicio,
  crearAsignacionServicio,
  type AsignacionServicio,
} from "@/lib/asignacionesServicio";
...
export function AsignacionServicioModal({
  clienteId,
  clienteNombre,
  servicio,
  subtipo,
  ...
  const hoy = new Date();
  const [historial, setHistorial] = useState<AsignacionServicio[]>([]);
  ...
  if (!cargado) {
    setCargado(true);
    getAsignacionesServicio(clienteId, servicio, subtipo).then(setHistorial);
  }

  function handleGuardar() {
    if (!nuevoResp) { setError("Seleccioná un responsable."); return; }
    setError("");
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex items-start justify-between px-6 py-5 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[13px] font-semibold text-gray-900">{clienteNombre}</p>
              <p className="text-[11px] text-gray-400 mt-0.5">Historial de responsables</p>
            </div>
          </div>
            ...
            {historial.length === 0 ? (
              <div className="text-sm text-gray-400 py-2">
                ...
                <p className="text-[12px] mt-1">
                  Responsable actual:{" "}
                  <span className="font-medium text-gray-700">{responsableActual ?? "—"}</span>
                </p>
              </div>
            ) : (
